...
function App({data_id}) {
                ...
                { data.data.map((index, key) => {
                    return(
                        <article class="blog_item"  key={key} >
                            ...
                            <a href={"/noti/".concat(index.id)}>
                                <img class="card-img rounded-0" src={index.imagen} alt="No existe la imagen" style={{height: 470}}/>
                            </a>
                            </div>
                            
                            <div class="blog_details">
                                <a class="d-inline-block" href={"/noti/".concat(index.id)}>
                                    <h2>{index.titulo}</h2>
                                </a>
                                <p>{index.resumen}</p>
                                <ul class="blog-info-link">
                                {index.interes.map((index, key) => (        
                                    <li key={key}><a href={"/noti/".concat(index.id)}>{index.nombre}</a></li>
                                ))}
                                </ul>
                            </div>
                        </article>
    ...
    async function getData(pageNumber = 1){
        if(data_id.id === "0"){
            const response = fetch('http://siadsis.uatf.edu.bo/api/notipag?page='.concat(pageNumber))
            .then(res => res.json())
            .then(
                (response) => {
            ...
            const response = fetch(CategoryApi.concat('noticat/').concat(data_id.id).concat('?page=').concat(pageNumber))
            .then(res => res.json())
            .then(
                (response) => {
                    // console.log(response)
    ...
    async function getCategory(pageNumber = 1){
        if(data_id.id === "0"){
            setCategory({nombre: 'General'})
        }
        else{
            const response = fetch(CategoryApi.concat('cat/').concat(data_id.id))
            .then(res => res.json())
            .then(
                (response) => {
                    // console.log(response)
...
if (document.getElementById('categories_index')) {
    const element =  document.getElementById('categories_index');
