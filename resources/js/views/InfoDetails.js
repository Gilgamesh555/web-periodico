...
function App({data_id}) {
    ...
    async function getData(pageNumber = 1){
        const response = fetch(CategoryApi.concat('info/').concat(data_id.id))
        .then(res => res.json())
        .then(
            (response) => {
...
if (document.getElementById('info_details')) {
    const element =  document.getElementById('info_details');
